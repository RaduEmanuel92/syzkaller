...
void loop()
{
	...
	for (int iter = 0;; iter++) {
		...
		//flag_fault_type = in_cmd[1];
		flag_fault_call = in_cmd[2];
		flag_fault_nth = in_cmd[3];
...
void execute_call(thread_t* th)
{
	...
	if (flag_inject_fault && th->call_index == flag_fault_call) {
		char buf[16];
		int n  = read(tid_fail_fd, buf, sizeof(buf) - 1);
		if (n <= 0)
			fail("failed to read /proc/self/task/tid/fail-nth");
		debug("tid_fail_fd=%d n=%d buf[0]=%d buf[1]=%d\n", tid_fail_fd, n, buf[0], buf[1]);
		th->fault_injected = n == 2 && buf[0] == '0' && buf[1] == '\n';
		buf[0] = '0';
		if (write(tid_fail_fd, buf, 1) != 1)
			fail("failed to write /proc/self/task/tid/fail-nth");
		close(tid_fail_fd);
